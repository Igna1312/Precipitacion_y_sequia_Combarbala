...
Para analizar la disponibilidad de recursos hídricos en la zona de estudio, se realizó un análisis exploratorio en dos estaciones meteorológicas: Cogotí (Comuna de Combarbalá) y Las Ramadas (Comuna de Monte Patria) durante el periodo 1970-2015. Este análisis incluye la creación de boxplots de precipitación mensual y líneas de tiempo del **Índice de Precipitación-Evapotranspiración Estandarizado (SPEI)** para identificar los periodos de sequía, este índice bajo el método de Hargreaves permite medir la sequía de un sector a partir de la comparación temporal de datos de precipitación mensual, temperatura máxima y minima (Beguería et al,. 2014). Para este estudio, se utilizó SPEI 12 (el cual mide sequia en un periodo de 12 meses consecutivos) y SPEI 3 (mide sequia en un periodo de 3 meses consecutivos), lo que permite observar los impactos de la sequía a diferentes escalas. 

## Área de Estudio
![Mapa elaborado con información del CR2 y los estudios de Aranda (1970) mediante Arcgis Pro](imagenes/mapa_estudio.png){fig-alt="mapa de combarbalá y monte patria"}
Las zonas de veranada comienzan desde los 1.700 metros sobre el nivel del mar (msnm) (precordillera), pero la mayoría de los campamentos se encuentra ya en la cordillera, entre los 2.200 y 3.400 (msnm). Es aquí donde se mueven los crianceros entre los meses de noviembre y abril (Castillo, 2003). Podemos observar que estos se ubican donde nacen los ríos y la mayoría de estos puestos se encuentra en Monte Patria (8), habiendo sólo 4 puestos en Combarbalá.
...
![Boxplot de precipitación en Las Ramadas](figuras/boxplot_las_ramadas.png){fig-alt="boxplot_ramadas" width="80%"}
![Boxplot de precipitación en Cogotí](figuras/boxplot_cogot'i.png){fig-alt="boxplot_cogoti" width="80%"}
...
![](imagenes/tabla_SPEI.jpg){fig-alt="Tabla de valores SPEI" width="50%"}
...
Aranda, X. (1970). Algunas consideraciones sobre la transhumancia en el norte chico.
...
Castillo, G. (2003). La vuelta de los años: reseñas y perspectivas sobre las comunidades, el pastoreo y la trashumancia en la región semiárida de Chile. Dinámicas de los sistemas agrarios en Chile árido: la región de Coquimbo, Santiago: LOM.
